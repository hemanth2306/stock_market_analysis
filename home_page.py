...
def home_menu(stock_data, closing_price):
    st.title("Yahoo Finance Stock Market Analysis and Predictions")
    st.markdown("Welcome to Yahoo Finance Stock Market Analysis and Predictions")

    home_menu = st.selectbox("Choose an option:", ["Analysis using a stock symbol", "View user manual or readme file", "View flow chart of the program"])

    if home_menu == "Analysis using a stock symbol":
    ...
    elif home_menu == "View user manual or readme file":
        st.markdown("You can view the user manual or readme file here.")
        readme_file = open('Readme.pdf', 'rb')
        st.write(readme_file.read())

    elif home_menu == "View flow chart of the program":
        st.image('flow_chart.png', caption='Flow Chart', use_column_width=True)
